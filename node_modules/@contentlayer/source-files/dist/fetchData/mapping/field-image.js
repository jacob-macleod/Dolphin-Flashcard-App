...
const getImageFieldData = ({ documentFilePath, contentDirPath, fieldDef, imagePath: imagePath_, }) => pipe(T.gen(function* ($) {
...
}), T.catchAll((error) => pipe(getFromDocumentContext('documentTypeDef'), T.chain((documentTypeDef) => T.fail(new FetchDataError.ImageError({
    error,
    documentFilePath,
    fieldDef,
...
const processImage = (fileBuffer) => T.gen(function* ($) {
    if (SharpModule === undefined && ImageScriptModule === undefined) {
        yield* $(pipe(T.tryPromise(() => import('sharp')), 
        // NOTE `sharp` is still a CJS module, so default import is needed
        T.tap((_) => T.succeedWith(() => (SharpModule = _.default))), T.catchAll(() => pipe(T.tryPromise(() => import('imagescript')), T.tap((_) => T.succeedWith(() => (ImageScriptModule = _))))), OT.withSpan('importSharpOrImageScript')));
    }
    if (SharpModule) {
...
const processImageWithImageScript = (fileBuffer) => pipe(T.gen(function* ($) {
    const format = ImageScriptModule.ImageType.getType(fileBuffer);
    if (format === null) {
        return yield* $(T.fail(new Error('Could not determine image type')));
    }
    const image = yield* $(pipe(T.tryPromise(() => ImageScriptModule.decode(fileBuffer)), OT.withSpan('decodeImage')));
    const { width, height } = image;
    image.resize(8, 8);
    const resizedData = yield* $(pipe(T.tryPromise(() => image.encode(70)), OT.withSpan('resizeImage')));
    return { resizedData, width, height, format };
}), OT.withSpan('processImageWithImageScript'));
const processImageWithSharp = (fileBuffer) => pipe(T.gen(function* ($) {
    ...
    const resizedInfo = yield* $(pipe(T.tryPromise(() => {
        const quality = 70;
        switch (format) {
            case 'jpeg':
