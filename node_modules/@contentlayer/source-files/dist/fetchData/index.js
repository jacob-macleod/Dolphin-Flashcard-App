...
export const fetchData = ({ coreSchemaDef, documentTypeDefs, flags, options, contentDirPath, contentDirInclude, contentDirExclude, skipCachePersistence = false, verbose, }) => {
    ...
    const fileUpdatesStream = pipe(FSWatch.makeAndSubscribe(watchPaths, {
        cwd: contentDirPath,
        ignoreInitial: true,
    ...
    const resolveParams = pipe(skipCachePersistence
        ? T.succeed(undefined)
        : core.DataCache.loadPreviousCacheFromDisk({ schemaHash: coreSchemaDef.hash }), T.either);
    return pipe(S.fromEffect(resolveParams), S.chainSwitchMapEitherRight((cache) => pipe(fileUpdatesStream, S.tapRight((e) => T.succeedWith(() => (e._tag === 'updated' || e._tag === 'deleted') && console.log(`\nFile ${e._tag}: ${e.relativeFilePath}`))), S.startWithRight(initEvent), S.mapEffectEitherRight((event) => pipe(event, T.matchTag({
    ...
    S.tapRight((cache_) => T.succeedWith(() => (cache = cache_))), S.tapRightEither((cache_) => skipCachePersistence
        ? T.unit
        : core.DataCache.writeCacheToDisk({ cache: cache_, schemaHash: coreSchemaDef.hash })))), S.mapEitherRight((cache) => embedReferences({ cache, coreSchemaDef })), S.mapEitherLeft((error) => new core.SourceFetchDataError({ error, alreadyHandled: error._tag === 'HandledFetchDataError' })));
};
...
const updateCacheEntry = ({ contentDirPath, filePathPatternMap, cache, event, flags, coreSchemaDef, options, contentTypeMap, }) => T.gen(function* ($) {
    yield* $(pipe(makeCacheItemFromFilePath({
    ...
    provideDocumentTypeMapState, These.effectTapSuccess((cacheItem) => T.succeedWith(() => {
        cache.cacheItemsMap[event.relativeFilePath] = cacheItem;
    })), These.effectTapErrorOrWarning((errorOrWarning) => FetchDataError.handleErrors({
