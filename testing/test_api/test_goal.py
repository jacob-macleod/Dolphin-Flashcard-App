...
class TestGoals(BaseApiActionsMixin):
    ...
    def test_update_goal_status(self, user):
        ...
        response = self.post_api(Routes.ROUTE_UPDATE_GOAL_STATUS["url"], {"userID": user["user_id"]})
    ...
    def test_update_goal_status_goal_xp_valid(self, user):
        xp_goal = create_xp_goal_no_db(user["user_id"], goalXP=51, endDate=date.get_current_date())
        request_data = {
            "userID": user["user_id"],
    ...
    def test_update_is_proper(self, user):
        ...
        xp_goal_1 = create_xp_goal(user["user_id"], goalXP=10, endDate=date.get_current_date())
        xp_goal_2 = create_xp_goal(user["user_id"], goalXP=11, endDate=date.get_current_date())
