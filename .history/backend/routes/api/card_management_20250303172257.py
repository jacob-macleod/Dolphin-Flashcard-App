    ...
    try:
        ...
        try:
            ...
            if not html_content.content:
                return jsonify({"error": "Unable to access Quizlet set - empty response"}), 400
            
            soup = BeautifulSoup(html_content.content, features='html.parser')
            
            if not soup or not soup.title:
                return jsonify({"error": "Unable to parse Quizlet set - invalid HTML content"}), 400
                
            flashcard_name = soup.title.text
            if not flashcard_name:
                return jsonify({"error": "Unable to parse Quizlet set - no title found"}), 400
        ...
        if not flashcard_description or not flashcard_name:
            return jsonify({"error": "Invalid or inaccessible Quizlet URL"}), 400
