...
def import_from_quizlet():
    ...
    try:
        ...
        #scrape the set from the url
        html_content = requests.get(url=quizlet_url, impersonate="chrome").content
 
        #parse the html content
        soup = BeautifulSoup(html_content, features='html.parser')
        
        #get the flashcard_name from parsed content
        flashcard_name = soup.title.text
        flashcard_description = ""
        
         #get the flashcard_description from parsed content
        desc_div = soup.find('div', class_='dcgy0px')
        if desc_div:
            flashcard_description = desc_div.get_text()
        #find all term-definition matches from 

        matches = re.findall(QUIZLET, str(soup), re.DOTALL)
        flashcards = {}

        cards = []
        print(flashcard_description, flashcard_name)
        ...
        print(user_id, folder, flashcard_name)
        flashcard_id = hash_to_numeric(user_id + folder + flashcard_name)
