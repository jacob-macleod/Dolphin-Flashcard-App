...
class TestQuizletImport(BaseApiActionsMixin):
    ...
    def test_invalid_url(self):
        ...
        try:
            ...
            response = self.post_api(Routes.ROUTE_IMPORT_FROM_QUIZLET["url"], test_data)
            assert False, "Should have raised an error for invalid URL"
        except AssertionError as e:
            # Should get a 500 status code with error message
            assert "500" in str(e)
            assert "Invalid Quizlet URL or unable to access the set" in str(e)
