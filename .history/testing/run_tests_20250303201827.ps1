...
null = Register-EngineEvent -SourceIdentifier PowerShell.Exiting -Action {
    Write-Host "Script is being terminated, cleaning up..." -ForegroundColor Yellow
    docker rm -f firestore-emulator 2>$null
    # Add any other cleanup tasks here
}

# Trap for Ctrl+C (SIGINT)
trap {
    Write-Host "Interrupted by user (Ctrl+C), cleaning up..." -ForegroundColor Yellow
    docker rm -f firestore-emulator 2>$null
    # Add any other cleanup tasks here
    exit 1
}
